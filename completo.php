...
<?php
    ...
    if($_POST) {
        ...
        if ($senha == $senhaConfirma) {
            $mensagem = "<span class='sucesso'><b>Sucesso</b>: As senhas são iguais: ".$senha."</span>".header('location:colaborador.php');
        } else {
            $mensagem = "<span class='erro'><b>Erro</b>: As senhas não conferem!</span>";
        }
        echo "<p id='mensagem'>".$mensagem."</p>";
        $stmt = $st->query("UPDATE usuario SET cd_senhausuario = '$senha'
                                          WHERE nm_emailusuario = '$email'");
$stmt->execute();
    }
?>
</main>
